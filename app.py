...
# --- BUILD: 2026-01-02-sync2 ---
# URL取り込み直後は on_change が発火しないことがあるため、クエリがあれば毎回同期してから判定する
try:
    _qp = st.query_params
    _has_incoming = bool(_qp.get("m","")) or bool(_qp.get("o",""))
    _mark = (st.session_state.get("mark_order") or "").strip()
    _store = st.session_state.get("odds_store", {}) or {}
    if _mark and _store:
        _m = parse_marks_order(_mark)
        _req = required_keys_for_mark(_m)
        for _k in _req:
            if _k in _store:
                st.session_state[f"odds_{_k}"] = float(_store[_k])
    if _has_incoming:
        on_mark_change()
except Exception:
    pass
...
st.caption("BUILD: 2026-01-02-sync2")
